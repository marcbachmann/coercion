var boolean, csv, date, dateRange, dateRangeRegex, dateRegex, integer, intersection, pagination, positiveInteger, sort,
...
sort = function(string, opt) {
  var isValid, isValidAsterisk, key, op, result, s, _i, _len, _ref, _ref1;
  if (opt == null) {
    opt = {};
  }
  opt.ascValue = opt.ascValue != null ? opt.ascValue : 'asc';
  opt.descValue = opt.descValue != null ? opt.descValue : 'desc';
  ...
  for (_i = 0, _len = _ref.length; _i < _len; _i++) {
    ...
    isValid = opt.allowed ? __indexOf.call(opt.allowed, key) >= 0 : true;
    isValidAsterisk = opt.allowed ? opt.allowed.reduce(function(acc, st) {
      var val;
      val = st.slice(st.length - 2) === '.*' ? st.slice(0, -2) : st;
    ...
    if (key && (isValid || isValidAsterisk)) {
      result[key] = op === '-' ? opt.descValue : opt.ascValue;
    }
  }
  return result;
};
